...
int main(int argc, char ** argv)
{
	...
	for (auto i = 0U; i < 5; ++i)
	{
		...
		{
			auto position = sf::Vector2f(k * 15, i * 15) + C;
			particles.push_back(std::make_shared < Particle >(position, position, sf::Vector2f(0.0f, 5.0f), r));
		}
	}
	...
	while (window.isOpen())
	{
		...
		{
			system.push(sf::Vector2f(0.0f, -0.50f));
		}
		...
		{
			system.push(sf::Vector2f(-0.50f, 0.0f));
		}
