#define _CRT_SECURE_NO_WARNINGS // нужно для ctime
...
//нет нормального преобразования std::filesystem::last_write_time в std::time_t в с++ 17
...
void DIR(const std::filesystem::path& path)
{
	if (std::filesystem::exists(path) && std::filesystem::is_directory(path))
	{
		for (const auto& entry : std::filesystem::directory_iterator(path))
		{
			...
			time.pop_back(); // нужно для того чтобы убрать /n в конце
